...
class CoprBuildTargetModel(GroupAndTargetModelConnector, Base):
    ...
    log_detective_runs = relationship("LogDetectiveRunModel", back_populates="copr_build_target")
...
class KojiBuildTargetModel(GroupAndTargetModelConnector, Base):
    ...
    log_detective_runs = relationship("LogDetectiveRunModel", back_populates="koji_build_target")
    ...
    def set_submitted_time(self, submitted_time: Optional[DateTime]):
        with sa_session_transaction(commit=True) as session:
    ...
    def __repr__(self):
        return f"KojiBuildTargetModel(id={self.id}, submitted_time={self.submitted_time})"
    ...
    def get_last_successful_scratch_by_commit_target(
        ...
        with sa_session_transaction() as session:
            return (
                ...
                .order_by(KojiBuildTargetModel.submitted_time.desc())
                .first()
            )
...
class SRPMBuildModel(ProjectAndEventsConnector, Base):
    ...
    def get_older_than(cls, delta: timedelta) -> Iterable["SRPMBuildModel"]:
        ...
        with sa_session_transaction() as session:
            return session.query(SRPMBuildModel).filter(
                SRPMBuildModel.submitted_time < delta_ago,
    ...
    def __repr__(self):
        return f"SRPMBuildModel(id={self.id}, submitted_time={self.submitted_time})"
...
class VMImageBuildTargetModel(ProjectAndEventsConnector, Base):
    ...
    def __repr__(self):
        return f"VMImageBuildTargetModel(id={self.id}, submitted_time={self.submitted_time})"
...
class LogDetectiveRunModel(GroupAndTargetModelConnector, Base):
    ...
    # for a build that doesn't exist anymore.
    copr_build_target_id = Column(
        Integer,
        ForeignKey("copr_build_targets.id", ondelete="CASCADE"),
    )
